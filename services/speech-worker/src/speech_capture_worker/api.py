...
def create_app(
    *,
    ...
    @app.put(
        ...
        response_model=SummaryRevisionDraftEnvelope,
        operation_id="saveJobSummaryRevisionDraft",
        tags=["summaries"],
        responses=PRIVATE_ERROR_RESPONSES,
    )
    def save_job_summary_revision_draft(
        job_id: str,
        revision_key: SafeIdentifier,
        request: SummaryRevisionDraftRequestSchema,
        principal: Principal,
        worker_store: Store,
        idempotency_key: Annotated[str, Header(alias="Idempotency-Key")],
    ) -> SummaryRevisionDraftEnvelope:
        _authorized_job(worker_store, principal, job_id)
        result = save_summary_revision_draft(
            worker_store,
            job_id,
            revision_key=revision_key,
        ...
        return SummaryRevisionDraftEnvelope(
            job=_job_schema(result.job),
            revision=SummaryRevisionSchema.model_validate(result.revision.to_dict()),
            saved=result.saved,
        )
    ...
    def get_job_publication_status(
        ...
        # Repair packages generated by earlier Worker versions as well as keep
        # the publication invariant explicit: a current receipt must describe
        # the current artifact manifest.  Superseded receipts remain available
        # in the append-only history table.
        worker_store.archive_stale_publication_receipt(
            job_id,
            current_manifest_sha256=plan.manifest_sha256,
