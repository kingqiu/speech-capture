...
NOTE_PROMPT_VERSION = "2026-08-27.1"
...
对问题排查、规则确认或方案评审类会议，优先按“问题现象与影响—已核实根因—规则或方案—
数据/实施处理—下一步验证”组织正文；同一事实不得换词复制到多个主题。凡原文明确出现数量、
比例、阈值、时间范围、优先顺序、表名、字段范围、输入输出或验收标准，必须保留在对应主题，
不能用“讨论了数据问题”“形成初步共识”等泛化表述替代。存在两层或多层判断、匹配、回退、
审批或处理规则时，必须按先后顺序逐层写清触发条件与结果，不能压缩成一句“优化流程”。
...
decisions 必须优先核对会上被明确确认、重复确认或用于后续执行的规则、阈值、数据范围、
优先级和取舍；“可以”“没问题”等只有在明确回应具体提议时，才能与该提议的证据共同构成决定。
待办应写成可交付、可验收的动作，尽量说明由谁做什么、何时完成以及交付物或验收结果；
交付物可写入 task，但必须有原文依据。原文没有负责人、期限、优先级或交付标准时留空或不写，
不能根据常识补成团队、P0/P1、日期或产物。
