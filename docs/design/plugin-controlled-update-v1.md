...
明确确认，以及退出后安装 helper、失败回滚和重开后加载校验均已完成；独立远程测试 Vault 的升级—回滚—
再升级闭环尚未执行
...
5. 用两个合成 Vault 覆盖正确 Vault、错误 Vault、旧备份重复 ID、损坏包、断网、空间不足、重启前中断、
   新版加载失败和手工恢复；（自动化矩阵已覆盖安装前/替换中故障、Vault 隔离、加载哈希拒绝和备份保留；
   新版完全无法加载时的人工恢复演练仍待独立测试 Vault 验证）
...
占用空间。新版完全无法加载时仍保留旧版整体备份，当前阶段继续以手工恢复工具作为最终兜底。
