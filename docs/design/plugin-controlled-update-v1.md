...
明确确认、退出后安装 helper、失败回滚、重开后加载校验，以及独立外部备份恢复工具均已完成。远端手工
升级/恢复闭环及宿主插件内完整路径已验收；最终 0.1.30 候选留在本地，按用户要求暂不逐版外发。
...
- 只在收到 Node `spawn` 事件后才报告助手已启动；同步异常和异步 `error` 均转为固定失败码，清理候选
  ZIP、helper 和请求，不把私人路径带入 UI。助手等待、失败等状态在设置页可见期间按秒读取当前事务；
  页面关闭时停止，过期异步结果不得覆盖新页面/事务。
- 助手等待用户退出后，在读取设置和移动活动目录前重新验证插件身份、版本、main 哈希及文件类型；
  不覆盖等待期间被另一个安装器修改的活动插件。
- 新增发布门用例：异步/同步启动失败、启动确认、单次确认 UI、失败自动显示、关闭页面后停止刷新、
  等待期间活动代码变化拒绝。完整发布门及宿主中断—错误提示—直接重试—退出后安装—重开验证通过。
- 宿主又完成 0.1.30 → 显式恢复开发基线 0.1.29 → 外部安装 0.1.30，分别在实际 Obsidian 中确认扫描与
  运行版本，最终为 0.1.30；设置哈希始终一致。全部在空白临时 Vault，未改动正式插件、会议或配对。
- 这关闭本阶段的本地开发/验收门；远端接收与升级由用户决定时机，合并一次发送。签名和 friend-ready
  预览属于后续独立范围，不作为本次个人 Alpha 的无限延期事项。
