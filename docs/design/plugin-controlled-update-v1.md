...
6. 在独立远程测试 Vault 完成一次 `N → N+1 → 回滚 N → 再升级 N+1`，并确认配对设置不变；
   （2026-09-19 用户截图已证明手工路径 0.1.25 → 0.1.28 → 0.1.25 → 0.1.28，重开后连接就绪；
   这不是插件内更新路径的验收，也不等同于逐字节 settings 校验。）
7. 插件内更新还须验证真实 Obsidian 父子进程关系、退出前不替换、自动重开及实际加载一致；通过后才把
   手工脚本从默认流程降为恢复工具。0.1.27/0.1.28 必须先经退出后的外部安装升级到修复版本，不得用旧 helper 自更新。
...
## 8. 2026-09-19 宿主 UI 验收发现与 0.1.29 修正

- 真实 Obsidian 启动的 helper 是其子孙进程。macOS `pgrep` 默认排除祖先，因此旧版 `pgrep -x Obsidian`
  会在应用仍运行时返回 1，提前替换活动文件。终端启动的 helper 和此前跳过等待的合成测试没有覆盖这个条件。
- 三个安装/恢复入口改为 `pgrep -a -x Obsidian`；只有返回 1 才认为已退出，查询错误一律停止。helper
  自动化新增等待状态、活动文件不变、查询错误阻断和退出时最后保存的设置保留测试。测试探针只允许在已有的
  `/private/tmp` smoke 事务中使用，正式路径固定系统 `pgrep`。
- 设置页合并原来的“确认候选”与“退出后安装”两次确认，保留一次明确的“确认更新，退出后安装”；不会主动
  强制退出应用，仍由用户正常 Command + Q 保存并退出。
- 宿主 Obsidian 1.13.7 使用无会议内容的临时 Vault 和回环合成发布服务实测。基线是嵌入本次修复的
  **开发版 0.1.28**（不是已分发的 0.1.28 字节），目标是完整发布门构建的 0.1.29。真实 UI 完成检查、
  下载、单次确认；退出前磁盘保持 0.1.28 且状态为 `waiting_for_exit`；正常退出后自动安装并重开，设置页
  显示“更新完成，已确认实际加载版本”，事务为 `loaded_verified`，main.js 哈希与目标发布清单一致。
- 同一次 UI 测试前后 `data.json` SHA-256 均为
  `97f520218ab092bb0483d236b6a32ca4bd6a9da7b45090668faf73beb5dc315e`。
  测试使用内存合成凭据，不写真实 Keychain，不请求真实会议，不修改正式 Vault 的插件或笔记。
- 已分发的旧 helper 无法靠下载新 ZIP 修复自身；0.1.28 → 0.1.29 必须在退出 Obsidian 后走一次外部安装。
  本轮未向远端发送 0.1.29，也未导入真实 Worker release store；远端插件内路径仍不冒充已验收。
