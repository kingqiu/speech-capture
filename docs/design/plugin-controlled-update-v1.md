...
状态：发布清单、不可变 Worker release store、宿主本机导入命令、认证只读 API，以及插件端只读检查、下载、
校验和明确确认入口已完成；退出后安装 helper 尚未实现
...
4. 实现最小 helper、事务日志、退出后安装、重开和回滚；
...
在第 6 步通过前，不把“检查更新”或“自动更新”展示为已经可用，也不替换当前经验证的手工安装恢复路径。
...
步骤 3 的实现额外固定：更新入口目前只出现在插件设置页，并明确标为“验证阶段”；它复用当前 Worker 的
