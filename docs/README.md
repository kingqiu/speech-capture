# Design Documentation

Read these documents in order:

1. [Product requirements](product-requirements.md)
2. [System architecture](architecture.md)
3. [Worker API direction](worker-api.md)
4. [Persistent Worker core](worker-core.md)
5. [Data model and output package](data-model-and-output.md)
6. [Security, privacy, and recovery](security-privacy-and-recovery.md)
7. [Testing strategy](testing-strategy.md)
8. [Local model spike](model-spike.md)
9. [UI direction](ui-direction.md)
10. [Decision log](decision-log.md)
11. [Roadmap](roadmap.md)
12. [Reference projects](references.md)

The decision log is the source of truth for accepted product choices. API and artifact schemas will become the source of truth for implementation contracts once they are added.
