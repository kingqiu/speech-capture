...
> **Project status:** backend personal alpha. Durable intake, queueing, resource preflight, progressive transcript persistence, deterministic audio normalization, restart-safe local ASR chunk execution, a durable whole-transcript alignment/completeness gate, anonymous speaker attribution, content-type classification and evidence-linked extraction, controlled forced-alignment fallback, conservative PCM evidence for uncovered ranges, evidence-bound definite-silence materialization, and explicit human-reviewed gap outcomes are implemented; there is no installable release or approved frontend yet.
